...
class Telas:
    ...
    def mudanca_cadastro_jogo(self):
        if validar_nome(self.nome_bixinho.get()) and validar_nome(self.nome_dono.get()):
            if validar_tamanho(self.nome_bixinho.get()) and validar_tamanho(self.nome_dono.get()):
                self.aviso = ''
                self.bixinho = Bixinho(date.today())
    ...
    def tela_status(self):
        ...
        self.status.after(2000, self.ficar_triste)
        self.status.after(1000, self.ficar_fome)
        self.status.after(3000, self.ficar_sujo)
        self.status.after(3000, self.sono_disposicao)
